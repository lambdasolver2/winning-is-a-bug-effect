...
- Server exposes the same routes twice: Node (`NodeHttpServer.layer`, local
  dev, no login) and Worker (`HttpRouter.toWebHandler` + `HttpServer.layerServices`).
- `apps/server/alchemy.run.ts`: one `Cloudflare.Worker` (`main:
